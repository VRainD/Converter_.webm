...
def test_mp3_audio_only_args(tmp_path: Path) -> None:
    _, _, args, warn = _base_args(OutputFormat.MP3, "out.mp3", True, tmp_path)
    assert "-vn" in args
...
def test_wav_audio_only_args(tmp_path: Path) -> None:
    _, _, args, _ = _base_args(OutputFormat.WAV, "out.wav", True, tmp_path)
    assert "-vn" in args
